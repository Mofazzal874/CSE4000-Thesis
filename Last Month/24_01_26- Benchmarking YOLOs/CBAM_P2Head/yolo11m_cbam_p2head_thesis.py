...
IMG_SIZE           = 640
BATCH_SIZE         = 8                            # P2 head: stride-4 maps are 4x larger
                                                  # spatially -> heavier VRAM than plain
                                                  # 11m. Original CBAM+P2 used batch=8 on
                                                  # T4 ("batch=16 OOMs"). Start at 8 to
                                                  # avoid a wasted OOM-retry cycle.
NOMINAL_BATCH      = 16                           # effective batch via grad-accum -- kept
...
# Verbatim CBAM+P2 architecture from the user's original
# 14-02-2026 (W-06)/YOLO11m_CBAM_P2Head.py (CELL 6): CBAM replaces C2PSA at
# backbone layer 10, and a 4th detection scale P2/4 (stride 4) is added so the
# head is Detect(P2,P3,P4,P5) over layers [19,22,25,28]. CBAM_REDUCTION /
# CBAM_KERNEL_SIZE are interpolated so the config block stays the single source.
...
backbone:
  ...
  - [-1, 1, SPPF, [1024, 5]]           # 9
  - [-1, 1, CBAM, [{CBAM_REDUCTION}, {CBAM_KERNEL_SIZE}]]  # 10 CBAM replaces C2PSA
...
def build_cbam_p2head_yaml(out_path: Path) -> Path:
    """Write the verbatim CBAM+P2 YAML and validate it has CBAM + 4 detect scales."""
    out_path.write_text(CBAM_P2HEAD_YAML.format(nc=NC))
    cfg = yaml.safe_load(out_path.read_text())
    detect = cfg["head"][-1]
    if len(detect[0]) != 4:
        raise RuntimeError(f"Expected 4 detection scales, got {len(detect[0])}")
    if not any(len(l) >= 3 and l[2] == "CBAM" for l in cfg["backbone"]):
        raise RuntimeError("CBAM not found in CBAM+P2 backbone")
    return out_path
...
def p2_head_metrics(model, dirs: Dict[str, Path], per_size_df: "pd.DataFrame"):
    """Section 11.6 (P2) detection-scale analysis. Records each detection head's
    stride + feature-map resolution + the object-size band it primarily serves,
    and pairs that with the already-computed per-size recall (per_size.csv) --
    which is the operative small-object evidence for the P2 head (very-tiny
    recall is where P2 pays off vs CBAM-only / baseline).

    NOTE: a fully rigorous *per-anchor-head AP* attribution is non-trivial under
    YOLO11's anchor-free dynamic (TaskAligned) assignment -- there is no fixed
    size->head rule and post-NMS boxes lose their head index. Per-size recall is
    the standard, defensible P2 metric and is reported here; true per-stride AP
    is left for the paper (logged as a documented [SKIPPED])."""
    arch_dir = dirs["arch"]
    stats: Dict[str, Any] = {}
    skips: List[str] = []
    try:
        det = model.model.model[-1]
        strides = [int(s) for s in det.stride.tolist()] if hasattr(det, "stride") else [4, 8, 16, 32]
        nl = len(strides)
        scale_names = ["P2", "P3", "P4", "P5"][:nl]
        # nominal object-size band each scale serves (sqrt-area, px), heuristic
        bands = {"P2": "<16", "P3": "16-32", "P4": "32-96", "P5": ">=96"}
        scales = []
        for name, s in zip(scale_names, strides):
            scales.append({
                "scale": name, "stride": s,
                "feat_res_at_640": f"{IMG_SIZE // s}x{IMG_SIZE // s}",
                "min_detectable_px_approx": s,           # grid cell ~ s px
                "primary_size_band_px": bands.get(name, "n/a"),
            })
        stats["n_detection_scales"] = nl
        stats["has_p2_head"] = (4 in strides) or (nl == 4)
        stats["detection_scales"] = scales
        # attach per-size recall (the operative metric) for convenience
        try:
            stats["per_size_recall"] = {
                r["bin"]: (None if pd.isna(r["recall"]) else float(r["recall"]))
                for _, r in per_size_df.iterrows()}
        except Exception:
            pass
        (arch_dir / "p2_detection_scales.json").write_text(json.dumps(stats, indent=2))
        skips.append("[SKIPPED] per-stride_AP (true per-head AP attribution) -- "
                     "anchor-free dynamic assignment; per_size.csv (very-tiny/tiny "
                     "recall) is the operative P2 metric. Full per-stride AP deferred "
                     "to the paper.")
        return stats, skips
    except Exception as e:
...
def train_with_oom_retry(model, base_kwargs: Dict[str, Any]) -> Any:
    ...
    for bs in OOM_RETRY_BATCHES:
        ...
        # round(nbs / batch), so setting nbs=NOMINAL_BATCH keeps the EFFECTIVE
        # batch fixed at 16 (== baseline/CBAM) regardless of the physical batch.
        kwargs["nbs"] = NOMINAL_BATCH
        _effective_accum = max(1, NOMINAL_BATCH // bs)
        torch.cuda.empty_cache(); gc.collect()
        try:
            print(f"[oom-retry] attempting batch={bs}, nbs={NOMINAL_BATCH} (effective accumulate={_effective_accum})")

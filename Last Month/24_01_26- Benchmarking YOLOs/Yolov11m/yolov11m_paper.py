...
PAPER-SCOPE variant. YOLO11m baseline on C2A under the FULL journal-grade
metric protocol defined in `Last Month/system_spec.md`. This run IS Phase A
row 1 (`yolo11m_baseline`, P0) of the headline ablation matrix in Section 9.6
/ Section 25.

This is the 5-SEED rigor version used for the journal paper (significance
testing, BCa CIs, paired bootstrap). For the THESIS report, use the sibling
`yolov11m_thesis.py`, which is identical in metric coverage but runs a single
seed per the thesis-month scope (`system_spec_thesis.md` Section 3.1). The two
files share the same metric/eval code on purpose -- only SEEDS and the doc
references differ.
...
SEEDS              = [0, 1, 2, 3, 4]              # PAPER rigor: 5 seeds (system_spec.md
                                                  # Sec 12.1) so paired-significance /
                                                  # BCa CIs are valid. Budget ~30-50 GPU-h
                                                  # PER SEED on the 4070 Ti SUPER, so this
                                                  # whole file is a multi-day run. For the
                                                  # thesis, run yolov11m_thesis.py (1 seed).
...
def main():
    print("\n" + "=" * 78)
    print(f"yolov11m_paper.py  --  MODEL_TAG={MODEL_TAG}  SEEDS={SEEDS}  [PAPER 5-seed]")

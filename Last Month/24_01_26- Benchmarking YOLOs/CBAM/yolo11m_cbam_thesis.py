"""
yolo11m_cbam_thesis.py
======================

THESIS-SCOPE variant. YOLO11m + CBAM on C2A. This is the ATTENTION-MODULE
ablation (`system_spec.md` Section 9.2): the original `C2PSA` attention block
in the YOLO11m backbone is replaced with a CBAM (Convolutional Block Attention
Module -- sequential channel + spatial attention). Everything else matches the
`yolo11m_baseline` run so the two are directly comparable.

HISTORICAL NOTE (verified 2026-05-31 from `01-02-2026- ablation study/`):
CBAM was run BEFORE the P2 head in the original study (CBAM: 2026-02-02..06;
P2Head: 2026-02-14..16), matching the progress-PDF generation order
(attention first, then detection head). So the natural ablation presentation
is: baseline -> +CBAM -> +P2 -> +CBAM+P2 -> +Mamba+CBAM+P2.

WHAT MAKES THIS FILE DIFFERENT FROM `yolo11m_thesis.py` (baseline):
  1. Architecture: builds from a C2PSA->CBAM-modified YAML and transfers the
     pretrained yolo11m.pt weights (see Section 2.5 below: CBAM module defs +
     namespace registration + `build_cbam_model()`). The CBAM module uses LAZY
     channel init (auto-detects channels at first forward) -- this is the exact
     implementation the user wrote in `6-2-26/results/cbam_module.py`, carried
     over verbatim so the saved weights stay loadable.
  2. Architecture-specific metrics (Section 11.6) are now ACTIVE, not skipped:
       - `attention_map_examples.png`  (spatial-attention heatmap overlays)
       - `channel_attention_weights.csv` (mean channel-attention vector)
       - `spatial_attention_entropy`   (mean entropy of the spatial map)
  3. MODEL_TAG = "yolo11m_cbam".

Metric coverage is otherwise IDENTICAL to `yolo11m_thesis.py` / the paper
version -- the full `system_spec.md` Section 11 catalog (detection /
training-dyn / efficiency / calibration / per-image / env / energy). Only P2 /
SSM / SAHI / TTA metrics remain [SKIPPED] (this model has none of those).

OPERATIONAL SCOPE (thesis):
  * SEEDS = [0]  -- 1 seed for ablation-chain models (system_spec_thesis.md
    Sec 3.1). The headline `mamba_cbam_p2head` uses 3 seeds; full 5-seed +
    paired significance is a PAPER task (use the *_paper.py variants).

Run order for the chain (one after another, never concurrent on one GPU):
  1. yolo11m_thesis.py             (baseline; SEEDS=[0])  -- DONE
  2. yolo11m_cbam_thesis.py        (this file; SEEDS=[0])
  3. yolo11m_p2head  thesis script (SEEDS=[0])
  4. yolo11m_cbam_p2head  "        (SEEDS=[0])
...
Spec compliance summary (sections deliberately implemented):
  ...
  - Section 25      : marked in_ablation_matrix=True (yolo11m_cbam attention ablation)
  - Section 26      : MODEL_CARD.md, ONNX export sanity

ACTIVE architecture-specific metrics (Section 11.6 -- CBAM):
  - attention_map_examples.png        (spatial-attention heatmap overlays)
  - channel_attention_weights.csv     (mean channel-attention vector)
  - spatial_attention_entropy         (mean entropy of spatial-attention map)
...
Explicitly SKIPPED (with reasons logged to skipped_metrics.txt):
  ...
  - paired-bootstrap p-values vs another model          (set BASELINE_PER_IMAGE_CSV
                                                         ...
                                                         per-image scores saved either way)
"""
...
MODEL_TAG          = "yolo11m_cbam"              # attention-module ablation (Sec 9.2)
...
LR0                = 0.001                        # diverged the P2 head. AdamW lr0=0.001 is
                                                  # the proven-stable C2A config (matches the
                                                  # Mamba-era scripts). One optimizer for all.
...
# =============================================================================
# 2.5 CBAM MODULE + REGISTRATION + MODEL BUILDER
#     (verbatim port of the user's 6-2-26/results/cbam_module.py, lazy-init)
# =============================================================================
# These classes MUST be registered into ultralytics' module + task namespaces
# at IMPORT TIME (not just before training) so that YOLO(best.pt) can
# deserialize the CBAM layers during the evaluation reload, and so DataLoader
# worker subprocesses (Windows 'spawn') that re-import this file also have them.
...
def register_cbam():
    """Register CBAM into ultralytics namespaces so YAML parse + checkpoint
    deserialization both find it. Idempotent. Also writes a sibling
    cbam_module.py for any DDP subprocess that imports by name (we run single
    GPU, but this keeps parity with the user's original approach)."""
    import ultralytics.nn.modules as _modules
    import ultralytics.nn.tasks as _tasks
    for ns in (_modules, _tasks):
        ns.CBAM = CBAM
        ns.ChannelAttention = ChannelAttention
        ns.SpatialAttention = SpatialAttention
    # also expose on globals used by torch.serialization safe-unpickling
    globals()["CBAM"] = CBAM

register_cbam()


def build_cbam_yaml(out_path: Path) -> Tuple[Path, int]:
    """Extract the stock YOLO11m architecture and replace every backbone
    `C2PSA` block with `[-1, 1, 'CBAM', [reduction, kernel_size]]`. Returns
    (yaml_path, n_replaced)."""
    ...
    for section in ("backbone", "head"):
        ...
        for i, layer in enumerate(cfg[section]):
            if len(layer) >= 3 and layer[2] == "C2PSA":
                cfg[section][i] = [layer[0], 1, "CBAM",
                                   [CBAM_REDUCTION, CBAM_KERNEL_SIZE]]
                n_rep += 1
    if n_rep == 0:
        raise RuntimeError("No C2PSA block found to replace with CBAM -- "
                           "ultralytics YOLO11m architecture may have changed.")
    out_path.write_text(yaml.safe_dump(cfg, sort_keys=False))
    return out_path, n_rep


def build_cbam_model(pretrained_weights: str, yaml_dir: Path):
    """Build a CBAM-modified YOLO11m and transfer pretrained yolo11m.pt weights.
    Mirrors the user's CELL 8: YOLO(cbam_yaml).load(yolo11m.pt)."""
    register_cbam()
    cbam_yaml, n_rep = build_cbam_yaml(yaml_dir / "yolov11m_cbam.yaml")
    if _IS_MAIN:
        print(f"[cbam] built CBAM YAML ({n_rep} C2PSA->CBAM) at {cbam_yaml}")
    model = YOLO(str(cbam_yaml))
    model.load(pretrained_weights)   # transfer matching pretrained weights
    if _IS_MAIN:
        print("[cbam] pretrained weights transferred onto CBAM architecture")
    return model


def cbam_attention_metrics(model, dirs: Dict[str, Path], run_id: str,
                           split: str = "test", n_examples: int = 6):
    """Section 11.6 (CBAM) architecture-specific metrics. Hooks every CBAM
    module, runs a few test images, and writes:
       architecture/channel_attention_weights.csv   (mean channel-attn vector)
       architecture/spatial_attention_entropy.json  (mean spatial-map entropy)
       architecture/attention_maps/*.png            (heatmap overlays)
       plots/<run_id>_cbam_channel_attention.png    (channel-weight bar plot)
    Returns (stats_dict, skipped_msgs, plot_index_rows). Never raises -- on any
    failure it degrades to a [SKIPPED] message so the run still completes."""
    import cv2
    arch_dir = dirs["arch"]
    plots_dir = dirs["plots"]
    amap_dir = arch_dir / "attention_maps"
    stats: Dict[str, Any] = {}
    skips: List[str] = []
    plot_rows: List[Tuple[str, str, str]] = []
    try:
        cbam_mods = [m for m in model.model.modules()
                     if m.__class__.__name__ == "CBAM"]
        if not cbam_mods:
            return {}, ["[SKIPPED] CBAM metrics -- no CBAM module found"], []
        amap_dir.mkdir(parents=True, exist_ok=True)
        captured: Dict[str, list] = {"channel": [], "spatial": []}
        ...
        def _hook(module, inp, out):
            try:
                if not getattr(module, "_initialized", False):
                    return
                x = inp[0]
                ca = module.channel_attention
                cw = ca.sigmoid(
                    ca.fc2(ca.relu(ca.fc1(ca.avg_pool(x)))) +
                    ca.fc2(ca.relu(ca.fc1(ca.max_pool(x)))))      # (B,C,1,1)
                captured["channel"].append(
                    cw.detach().float().cpu().mean(dim=0).flatten())
                xc = ca(x)
                sa = module.spatial_attention
                avg_o = torch.mean(xc, dim=1, keepdim=True)
                max_o, _ = torch.max(xc, dim=1, keepdim=True)
                sm = sa.sigmoid(sa.conv(torch.cat([avg_o, max_o], dim=1)))  # (B,1,H,W)
                captured["spatial"].append(sm.detach().float().cpu())
            except Exception:
                pass

        handles = [m.register_forward_hook(_hook) for m in cbam_mods]

        img_dir = DATASET_ROOT / split / "images"
        images = sorted(img_dir.glob("*.png"))[:n_examples]
        chan_vecs, entropies = [], []
        for img_path in images:
            captured["channel"].clear(); captured["spatial"].clear()
            _ = model.predict(str(img_path), conf=0.25, verbose=False)
            if captured["channel"]:
                chan_vecs.append(captured["channel"][0].numpy())
            if captured["spatial"]:
                sm = captured["spatial"][0][0, 0].numpy()  # (H,W) in [0,1]
                # spatial entropy: normalise map to a prob dist, Shannon entropy
                pmap = sm.flatten().astype(np.float64)
                pmap = pmap / (pmap.sum() + 1e-12)
                ent = float(-(pmap * np.log(pmap + 1e-12)).sum())
                entropies.append(ent)
                # overlay heatmap on the image
                im = cv2.imread(str(img_path))
                if im is not None:
                    H, W = im.shape[:2]
                    hm = cv2.resize((sm * 255).astype(np.uint8), (W, H))
                    hm = cv2.applyColorMap(hm, cv2.COLORMAP_JET)
                    overlay = cv2.addWeighted(im, 0.6, hm, 0.4, 0)
                    cv2.imwrite(str(amap_dir / f"{img_path.stem}_cbam_spatial.png"),
                                overlay)

        for h in handles:
            h.remove()

        # channel-attention weights CSV + bar plot
        if chan_vecs:
            maxlen = max(len(v) for v in chan_vecs)
            padded = np.vstack([np.pad(v, (0, maxlen - len(v)))
                                for v in chan_vecs])
            mean_vec = padded.mean(axis=0)
            cdf = pd.DataFrame({"channel_idx": np.arange(len(mean_vec)),
                                "mean_attention": mean_vec})
            cdf.to_csv(arch_dir / "channel_attention_weights.csv", index=False)
            stats["channel_attention_mean"] = float(mean_vec.mean())
            stats["channel_attention_std"]  = float(mean_vec.std())
            fig, ax = plt.subplots(figsize=(10, 3))
            ax.bar(cdf["channel_idx"], cdf["mean_attention"])
            ax.set_xlabel("channel index"); ax.set_ylabel("mean attention")
            ax.set_title("CBAM channel-attention (mean over sample, test)")
            fig.tight_layout()
            pf = f"{run_id}_cbam_channel_attention.png"
            fig.savefig(plots_dir / pf, dpi=200, bbox_inches="tight")
            plt.close(fig)
            plot_rows.append((pf, "architecture/channel_attention_weights.csv",
                              "channel_idx,mean_attention"))
        else:
            skips.append("[SKIPPED] channel_attention_weights -- no activations captured")

        if entropies:
            stats["spatial_attention_entropy_mean"] = float(np.mean(entropies))
            stats["spatial_attention_entropy_std"]  = float(np.std(entropies))
            stats["spatial_attention_entropy_per_image"] = [float(e) for e in entropies]
            (arch_dir / "spatial_attention_entropy.json").write_text(
                json.dumps(stats, indent=2))
            stats["attention_map_examples"] = len(list(amap_dir.glob("*.png")))
        else:
            skips.append("[SKIPPED] spatial_attention_entropy -- no maps captured")

        stats["cbam_modules_found"] = len(cbam_mods)
        return stats, skips, plot_rows
    except Exception as e:
        return ({}, [f"[SKIPPED] CBAM attention metrics -- {e}"], [])
...
def smoke_test() -> bool:
    ...
    try:
        ...
        # 2. Tiny train -- build the CBAM-modified architecture (exercises the
        #    C2PSA->CBAM YAML edit + registration + weight transfer paths too).
        m = build_cbam_model(PRETRAINED_WEIGHTS, smoke_dir)
        kw = dict(
            data=str(smoke_yaml),
            epochs=SMOKE_EPOCHS,
            imgsz=IMG_SIZE,
...
def write_env_json(path: Path, *, run_id: str, seed: int,
    ...
    env = {
        ...
        "matrix_status":      ("Attention-module ablation (yolo11m_cbam) -- "
                               "C2PSA->CBAM, per system_spec.md Section 9.2"),
...
def run_one_seed(seed: int) -> Dict[str, Any]:
    ...
    # CBAM architecture-specific metrics (Section 11.6) -- ACTIVE for this model
    cbam_stats, cbam_skips, cbam_plot_rows = cbam_attention_metrics(
        model, dirs, run_id, split="test", n_examples=6)
    ...
    # Architecture-specific skips. CBAM metrics are ACTIVE (any partial misses
    # are reported via cbam_skips). P2 / SSM / SAHI / TTA remain N/A here.
    skipped.extend(cbam_skips)
    skipped.extend([
        "[SKIPPED] per-stride_AP                  -- no P2 head in yolo11m_cbam",
        "[SKIPPED] tiny_obj_recall_by_head        -- no P2 head in yolo11m_cbam",
        "[SKIPPED] ssm_state_norm                 -- no SSM in yolo11m_cbam",
        "[SKIPPED] forward_vs_backward_scan_disag -- no SSM in yolo11m_cbam",
        "[SKIPPED] window_size_per_layer          -- no SSM in yolo11m_cbam",
        "[SKIPPED] dilation_branch_contribution   -- no SSM in yolo11m_cbam",
        "[SKIPPED] injection_layer_indices        -- no SSM in yolo11m_cbam",
    ...
    # CBAM channel-attention plot provenance (Section 11.6 / 14)
    for pf, dfile, cols in cbam_plot_rows:
        pidx.add(pf, dfile, cols, __file__, run_id)
    pidx.finalize()
...
**Status:** Attention-module ablation (`yolo11m_cbam`) per system_spec.md
Section 9.2 -- the backbone C2PSA block is replaced with CBAM (sequential
channel + spatial attention, reduction={CBAM_REDUCTION}, kernel={CBAM_KERNEL_SIZE}).
Compared directly against the `yolo11m_baseline` run (same data/seed/protocol).
...
## Limitations
- THESIS scope: generated from SEEDS={SEEDS} (1 seed for ablation-chain
...
""")
    ...
    manifest = {
        "run_id":            run_id,
        "model_tag":         MODEL_TAG,
        "matrix_status":     "Attention-module ablation (yolo11m_cbam, C2PSA->CBAM) per Sec 9.2",
        "parent_run_id":     None,
        "git_commit":        GIT_HASH,
        "git_dirty":         GIT_DIRTY,
...
def main():
    print("\n" + "=" * 78)
    print(f"yolo11m_cbam_thesis.py  --  MODEL_TAG={MODEL_TAG}  SEEDS={SEEDS}  [THESIS 1-seed, C2PSA->CBAM]")
    ...
    if len(all_summaries) >= 1:
        ...
        if n_done < n_req:
            print(f"[rollup] WARN: only {n_done}/{n_req} seed(s) succeeded; "
                  f"re-run the failed seed(s).")
        print("[rollup] NOTE: THESIS scope runs 1 seed (ablation chain) / 3 seeds "
              "(headline). Variance + paired-significance testing is deferred to "
              "the paper -- use yolov11m_paper.py (5 seeds) per system_spec.md "
              "Section 12.1 before making any significance claim.")

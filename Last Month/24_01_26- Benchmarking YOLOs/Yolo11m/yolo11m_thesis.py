...
THESIS-SCOPE variant. YOLO11m baseline on C2A. This is step 1 of the thesis
additive ablation chain (baseline -> +P2 -> +CBAM+P2 -> +Mamba+CBAM+P2) in
`Last Month/system_spec_thesis.md` Section 3.

Metric coverage is IDENTICAL to `yolov11m_paper.py` -- the full metric catalog
from `system_spec.md` Section 11 (detection / training-dyn / efficiency /
calibration / per-image / env / energy, with [SKIPPED] entries for the
architecture-specific metrics that don't apply to vanilla 11m). The user asked
to keep the same performance metrics as the paper version and only change the
operational scope per `system_spec_thesis.md`.

The ONLY operational differences vs `yolov11m_paper.py`:
  * SEEDS = [0]  -- 1 seed for the ablation-chain models (this baseline is a
    chain model). The headline model `mamba_cbam_p2head` uses 3 seeds; the
    full 5-seed protocol + paired significance is deferred to the paper
    (`system_spec_thesis.md` Section 3.1 / Section 6.1).
  * Doc references point at `system_spec_thesis.md`; significance-test warnings
    are reframed (significance is a paper-phase task, not a thesis blocker).

Run order for the chain (one after another, never concurrent on one GPU):
  1. yolov11m_thesis.py            (this file; SEEDS=[0])
  2. yolo11m_p2head   thesis script (SEEDS=[0])
  3. yolo11m_cbam_p2head  "        (SEEDS=[0])
  4. mamba_cbam_p2head    "        (SEEDS=[0,1,2]  <- headline, 3 seeds)
...
SAVE_PERIOD        = 25                           # epochs; checkpoint cadence (Section 20.2)
                                                  # (raised 5->25: Ultralytics saves last.pt
                                                  # EVERY epoch so resume is unaffected; this
                                                  # just cuts the ~110MB epochN.pt snapshots
                                                  # from 60 to ~12 per run -- big disk saving.)
...
COS_LR             = True
LRF                = 0.01
OPTIMIZER          = "AdamW"                      # PINNED. Was Ultralytics auto -> MuSGD(lr=0.01),
LR0                = 0.001                        # which DIVERGED the heavier P2 head (val cls
                                                  # loss -> inf ~epoch 50). AdamW lr0=0.001 is the
                                                  # proven-stable C2A config; pin it for ALL models
                                                  # so the ablation shares one optimizer.
...
def architecture_report(model: YOLO, arch_dir: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        # NOTE: a checkpoint loaded for eval has requires_grad=False on every
        # param, which would wrongly report 0 trainable. Count all params except
        # the frozen DFL conv (the only layer Ultralytics freezes) instead.
...
def run_one_seed(seed: int) -> Dict[str, Any]:
    ...
    if DO_TRAIN:
        if resume and ultra_last.is_file():
            # TRUE resume: load FROM the checkpoint so Ultralytics restores the
            # epoch counter + optimizer state. Passing resume=True to a freshly
            # built model does NOT resume (Ultralytics rejects it as "not a
            # resumable training checkpoint" and silently restarts from epoch 1).
    ...
    eff = {
        ...
        # -- this is the number to report in the paper, NOT the forward-only one below.
        ...
        # Forward-only latency (model.model(x), no NMS/pre/post) -- much smaller.
...
def main():
    print("\n" + "=" * 78)
    print(f"yolov11m_thesis.py  --  MODEL_TAG={MODEL_TAG}  SEEDS={SEEDS}  [THESIS 1-seed]")

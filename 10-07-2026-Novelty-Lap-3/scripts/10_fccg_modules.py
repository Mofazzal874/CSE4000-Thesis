...
CONTROL_YAML_NAME = "yolo11m_cbam_p2.yaml"   # S1 control arm (base CBAM+P2, NO FCCG)
...
# --------------------------------------------------------------------------
# S1 CONTROL yaml — base YOLO11m + CBAM + P2 head, NO FCCG (plain nn.Upsample,
# no FCCGFuse). Identical detection head to FCCG (Detect over P2,P3,P4,P5 with
# channels [128,256,512,512]) so the two arms differ ONLY by the FCCG insertion.
# Verbatim structure from the thesis yolov11m_cbam_p2head.yaml lineage.
# --------------------------------------------------------------------------
CONTROL_YAML = """# YOLO11m + CBAM + P2 head (S1 control arm) — NO FCCG
nc: 1
scales:
  m: [0.50, 1.00, 512]
...
head:
  ...
  - [[19, 22, 25, 28], 1, Detect, [1]]           # 29 Detect(P2,P3,P4,P5)
"""
...
def emit_control_yaml() -> Path:
    out = Path(__file__).with_name(CONTROL_YAML_NAME)
    out.write_text(CONTROL_YAML, encoding="utf-8")
    print(f"[emit] wrote {out}")
    return out

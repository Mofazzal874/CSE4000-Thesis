...
================================================================================
ThunderCompute A100 80GB — Single GPU Optimized Version

Trains ONE new model:
  yolo11m_atrousmamba_cbam_p2head

Optionally loads pre-trained for comparison (if uploaded):
...
Architecture:
  ...
  Neck     : C3K2Mamba with AtrousSSM (dilated window SSM, d=[1,2,4])
  Head     : P2 4-scale detection
...
DATA_DIR = os.path.expanduser("~/data")             # Dataset root
...
# ThunderCompute pre-installs PyTorch 2.9+cu128 — typically no mismatch
_needs_upgrade = (
    _driver_cuda.startswith("12.6") and "cu124" in _torch_ver
)
if _needs_upgrade:
    print("  ⚠  MISMATCH detected — reinstalling PyTorch cu126 …")
...
# TF32: Uses TensorFloat-32 for float32 matmuls — ~3x faster, negligible precision loss
torch.backends.cuda.matmul.allow_tf32 = True
torch.backends.cudnn.allow_tf32 = True
# cuDNN benchmark: auto-selects fastest convolution algorithm for input sizes
torch.backends.cudnn.benchmark = True
# Set float32 matmul precision to 'high' (uses TF32 on A100)
...
# Single GPU — no DDP needed on A100 80GB
DEVICE = "0" if num_gpus >= 1 else "cpu"
print(f"GPU: {gpu_name} ({gpu_mem:.1f} GB)  |  DEVICE: {DEVICE}")

if "A100" in gpu_name:
    print("  ✓ A100 detected — full BF16 + TF32 support")
elif num_gpus > 0:
    print(f"  ⚠ Not A100 ({gpu_name}) — config may need adjustment")
...
# ============================================================================
print(f"\nSearching for C2A dataset in {DATA_DIR}/ …")
DATASET_ROOT = None
...
if DATASET_ROOT is None:
    print(f"  ❌ C2A dataset NOT found. Contents of {DATA_DIR}/:")
    if os.path.exists(DATA_DIR):
        for _root, _dirs, _files in os.walk(DATA_DIR):
            _lvl = _root.replace(DATA_DIR, "").count(os.sep)
            if _lvl < 4:
                print("  " * _lvl + f"  📁 {os.path.basename(_root)}/")
                for _f in _files[:3]:
                    print("  " * _lvl + f"    📄 {_f}")
    raise FileNotFoundError(
        f"C2A dataset not found under {DATA_DIR}/.\n"
        "Make sure you uploaded and extracted the dataset.\n"
        "Expected structure: {DATA_DIR}/<some_folder>/train/images/\n"
        "See the setup guide for dataset download instructions."
    )
...
dataset_yaml_path = os.path.join(BASE_DIR, "c2a.yaml")
...
print(f"✓ Dataset YAML written: {dataset_yaml_path}")

# ── Locate comparison models (optional) ─────────────────────────────────────
if OLD_MAMBA_BEST:
    OLD_MAMBA_BEST = os.path.expanduser(OLD_MAMBA_BEST)
    if not os.path.exists(OLD_MAMBA_BEST):
        print(f"  ⚠  Old Mamba model NOT found: {OLD_MAMBA_BEST}")
        OLD_MAMBA_BEST = None
    else:
        print(f"  ✓ Old Mamba+CBAM+P2 found: {OLD_MAMBA_BEST}")
else:
    OLD_MAMBA_BEST = None
    print("  ⚠  Old Mamba+CBAM+P2 not configured — skipping comparison")

if CBAM_P2_BEST:
    CBAM_P2_BEST = os.path.expanduser(CBAM_P2_BEST)
    if not os.path.exists(CBAM_P2_BEST):
        print(f"  ⚠  CBAM+P2 model NOT found: {CBAM_P2_BEST}")
        CBAM_P2_BEST = None
    else:
        print(f"  ✓ CBAM+P2 found: {CBAM_P2_BEST}")
else:
    CBAM_P2_BEST = None
    print("  ⚠  CBAM+P2 not configured — skipping comparison")
...
_cbam_path = os.path.join(BASE_DIR, "cbam_module.py")
with open(_cbam_path, "w") as f:
    f.write(cbam_code)
exec(cbam_code)
...
# ============================================================================
# CELL 5: AtrousSSM Modules (JIT-Optimized Selective Scan)
# ============================================================================
...
# ── JIT-compiled selective scan (A100 optimization) ─────────────────────────
# The sequential scan loop is the main bottleneck.
# JIT compilation eliminates Python loop overhead → significant speedup.

def _selective_scan_loop(
    deltaA: torch.Tensor,      # (B_win, L, D, N)
    deltaB_u: torch.Tensor,    # (B_win, L, D, N)
    C_param: torch.Tensor,     # (B_win, L, N)
    B_win: int, L: int, D: int, N: int
) -> torch.Tensor:
    """SSM recurrence: x[i] = deltaA[i]*x[i-1] + deltaB_u[i], y[i] = (x[i]*C[i]).sum()"""
    x = torch.zeros(B_win, D, N, device=deltaA.device, dtype=torch.float32)
...
# Try JIT compilation — if it fails (prototyping mode GPU-over-TCP), use Python fallback
try:
    _ssm_scan_fn = torch.jit.script(_selective_scan_loop)
    # Warm up JIT with a small tensor to verify it works
    _test_dA = torch.zeros(1, 2, 4, 4)
    _test_dBu = torch.zeros(1, 2, 4, 4)
    _test_C = torch.zeros(1, 2, 4)
    _ = _ssm_scan_fn(_test_dA, _test_dBu, _test_C, 1, 2, 4, 4)
    del _test_dA, _test_dBu, _test_C
    print("✓ JIT-compiled selective scan (optimized for A100)")
except Exception as _jit_err:
    _ssm_scan_fn = _selective_scan_loop
    print(f"⚠ JIT unavailable ({_jit_err}) — using Python scan (slower)")
...
class AtrousSSM(nn.Module):
    ...
    Extends local-window SSM with dilated scanning.
    """
...
def run_smoke_test():
    ...
    if device == "cuda":
        ...
        with torch.amp.autocast("cuda"):
            y = blk(x)
        assert torch.isfinite(y).all(), "NaN in AMP forward!"
    ...
    if device == "cuda":
        print(f"\n[8/8] Memory estimate (layer-13 equivalent, batch={BATCH_SIZE}) …")
        torch.cuda.reset_peak_memory_stats()
        blk = C3K2Mamba(1024, 512, n=2, d_state=D_STATE, dilations=DILATIONS).to(device)
        _test_bs = min(BATCH_SIZE, 16)  # don't allocate full batch for test
        x = torch.randn(_test_bs, 1024, 40, 40, device=device)
        with torch.amp.autocast("cuda"):
            y = blk(x)
        y.sum().backward()
        peak_mb = torch.cuda.max_memory_allocated() / 1024**2
        total_budget = gpu_mem * 1024
        print(f"    Peak VRAM (batch={_test_bs}): {peak_mb:.0f} MB  (GPU budget: {total_budget:.0f} MB)  ✓")
...
# ============================================================================
# CELL 7: Register CBAM + C3K2Mamba in Ultralytics (YAML-native)
# ============================================================================
#
# Even without DDP, ultralytics YAML parser needs to know about C3K2Mamba.
# We register it in the ultralytics namespace so parse_model() builds it.
#
import site as _site

# ── Step 1: Write atrous_mamba_module.py (for ultralytics import) ──────────
# This file includes the JIT scan so it works consistently when imported.
...
def _selective_scan_loop(
    deltaA: torch.Tensor, deltaB_u: torch.Tensor,
    C_param: torch.Tensor, B_win: int, L: int, D: int, N: int
) -> torch.Tensor:
    x = torch.zeros(B_win, D, N, device=deltaA.device, dtype=torch.float32)
    ys = torch.empty(B_win, L, D, device=deltaA.device, dtype=torch.float32)
    for i in range(L):
...
_atrous_path = os.path.join(BASE_DIR, "atrous_mamba_module.py")
with open(_atrous_path, "w") as f:
    f.write(_atrous_module_code)
print("✓ atrous_mamba_module.py written")

# ── Step 2: Copy modules to site-packages ──────────────────────────────────
_installed_sp = None
for _sp in _site.getsitepackages():
    try:
        shutil.copy(_cbam_path, os.path.join(_sp, "cbam_module.py"))
        shutil.copy(_atrous_path, os.path.join(_sp, "atrous_mamba_module.py"))
        print(f"  ✓ Modules → site-packages: {_sp}")
        _installed_sp = _sp
        break
    except Exception:
        continue
...
if _installed_sp is None:
    print("  ⚠  Could not install to site-packages — trying sys.path fallback")
...
if _tasks_changed:
    with open(_tasks_file, "w") as _f:
        _f.write(_tasks_src)
    print(f"  ✓ Patched ultralytics/nn/tasks.py")
...
if _modules_changed:
    ...
    print(f"  ✓ Patched ultralytics/nn/modules/__init__.py")
...
if _patched_parse:
    ...
    print(f"  ⚠ Could not patch parse_model — will verify during dry-run")
...
# ============================================================================
# CELL 8: YAML + Gradient Clipping + Dry-Run
# ============================================================================

# NOTE: inject_mamba_neck is defined for evaluation model loading fallback
...
_yaml_path = os.path.join(BASE_DIR, "yolov11m_atrousmamba_cbam_p2head.yaml")
_base_yaml_path = os.path.join(BASE_DIR, "yolov11m_cbam_p2head.yaml")
with open(_yaml_path, "w") as f:
    f.write(_ATROUS_YAML)
with open(_base_yaml_path, "w") as f:
    f.write(_BASE_YAML)
...
class NaNStopCallback:
    ...
    def on_train_batch_end(self, trainer):
        ...
        if loss is not None and not torch.isfinite(loss).all():
            print(f"\n⚠ NaN/Inf loss detected at epoch {trainer.epoch+1}, "
                  f"batch {trainer.batch_idx}!")
...
class F2EarlyStopCallback:
    """Stops training if F2 score doesn't improve for `patience` epochs."""
    ...
    def on_fit_epoch_end(self, trainer):
        ...
        if self.wait >= self.patience:
            print(f"\n⚠ F2 early stop: no improvement for {self.patience} epochs "
                  f"(best F2={self.best_f2:.4f})")
            trainer.stop = True


class GradientMonitorCallback:
    """Tracks gradient norms each epoch."""
    def __init__(self):
        self.norms = []
    ...
    def on_train_epoch_end(self, trainer):
        ...
        if (trainer.epoch + 1) % 5 == 0:
            print(f"  [Grad] epoch {trainer.epoch+1}: norm={total_norm:.2f}")

    def on_train_end(self, trainer):
        if self.norms:
            csv_path = os.path.join(BASE_DIR, "gradient_norms.csv")
            pd.DataFrame({"epoch": range(1, len(self.norms)+1),
                          "grad_norm": self.norms}).to_csv(csv_path, index=False)
            print(f"  Gradient norms saved: {csv_path}")


class SessionCheckpointManager:
    """
    Saves checkpoints to CKPT_DIR for session recovery.
    ThunderCompute instances are ephemeral — download checkpoints before deleting!
    """
    def __init__(self, checkpoint_every: int = 5,
                 working_dir: str = None):
        self.every       = checkpoint_every
        self.working_dir = Path(working_dir or CKPT_DIR)
    ...
    def _print_checkpoint_info(self, epoch: int):
        print("\n" + "-" * 65)
        print(f"  CHECKPOINT SAVED  (epoch {epoch})")
        print(f"  Files: {self.working_dir}/session_last.pt")
        print(f"         {self.working_dir}/session_meta.json")
        print(f"  Download via: tnr scp 0:{self.working_dir}/ ./checkpoints/")
        print("-" * 65 + "\n")
    ...
    def on_fit_epoch_end(self, trainer):
        ...
        if copied_last:
            self._print_checkpoint_info(epoch)
...
# ============================================================================
# CELL 10: Training (Single GPU, OOM-safe, session-resumable)
# ============================================================================
from ultralytics import YOLO
...
print(f"  GPU: {gpu_name}  |  Batch: {BATCH_SIZE}  |  Workers: 8")
print("=" * 80)
...
ATROUS_RUN_DIR = os.path.join(BASE_DIR, "runs/detect/yolo11m_atrousmamba_cbam_p2head")
...
def _verify_injection_callback(trainer):
    ...
    print(f"  Status: {'✓ AtrousSSM ACTIVE' if has_mamba else '✗ MISSING!'}")
    print(f"{'='*70}\n")
...
if RESUME_TRAINING and not RESUME_PT:
    ...
    for _search_dir in [CKPT_DIR, BASE_DIR]:
        _candidate = os.path.join(_search_dir, "session_last.pt")
        if os.path.exists(_candidate):
            RESUME_PT = _candidate
            print(f"  ✓ Found: {RESUME_PT}")
            break
    if not RESUME_PT:
        raise FileNotFoundError(
            f"Auto-detect failed: no session_last.pt in {CKPT_DIR}\n"
            "Upload your checkpoint and set RESUME_PT manually.")

if RESUME_TRAINING and RESUME_PT:
    RESUME_PT = os.path.expanduser(RESUME_PT)
    ...
    if _has_mamba:
        ...
        print(f"    → YAML-native registration should rebuild during resume")
...
# ── FRESH TRAINING with OOM retry ────────────────────────────────────────────
    ...
    for _batch_attempt in OOM_RETRY_BATCHES:
        ...
        if _n_mamba == 0:
            raise RuntimeError("YAML parsed but C3K2Mamba NOT found!")
        ...
        try:
            ...
            _trained_ok = True
            _train_time = (time.time() - _epoch_start_time) / 3600
            print(f"\n✓ Training complete (batch_size={_batch_attempt}, {_train_time:.1f}h)")
            break
            ...
            print(f"\n  OOM at batch_size={_batch_attempt}: {type(_oom).__name__}")

            _partial_last = Path(ATROUS_RUN_DIR) / "weights" / "last.pt"
            if _partial_last.exists():
                print(f"  Partial checkpoint found: {_partial_last}")
                shutil.copy(str(_partial_last), os.path.join(CKPT_DIR, "session_last.pt"))
            ...
            if _batch_attempt == OOM_RETRY_BATCHES[-1]:
                raise RuntimeError(
                    f"OOM even at batch_size={_batch_attempt}. "
                    "All retry options exhausted.\n"
                    "Options:\n"
                    "  1. Reduce dilations from [1,2,4] to [1,2] in config.\n"
                    "  2. Reduce d_state from 4 to 2.\n"
                    "  3. Reduce imgsz from 640 to 512.\n"
                ) from _oom
...
REPORT_DIR = os.path.join(BASE_DIR, "benchmark_reports")
...
if len(runs) >= 2:
    ...
    print(f"  ✓ {len(runs)}-way overlay plot saved")
...
def _load_atrous_model(weights_path: str, label: str = "model",
    ...
    print(f"⚠ {label}: C3K2Mamba missing ({n_params/1e6:.2f}M)")
    ...
    try:
        ...
        if mamba_keys_missing:
            print(f"  ⚠ {len(mamba_keys_missing)} SSM keys missing — using base model")
            return model
...
if complexity_rows:
    ...
    if len(cdf) > 1:
        ...
        cdf["Delta Params(%)"] = ((cdf["Params(M)"] / base) - 1) * 100
...
if models_for_eval:
    ...
    official_test = {}
    for name, model in models_for_eval.items():
        res = model.val(data=dataset_yaml_path, split="test", verbose=False, plots=True)
        official_test[name] = {
...
def make_comparison_table(summaries: list, split: str) -> pd.DataFrame:
    metrics = [
        ...
        ("F1",              "F1"),
        ("F2",              "F2"),
        ("very_tiny_recall","Very Tiny Recall (<64px2)"),
        ("tiny_recall",     "Tiny Recall (64-256px2)"),
        ("small_recall",    "Small Recall (256-1024px2)"),
        ("medium_recall",   "Medium Recall (1024-9216px2)"),
...
report_lines = [
    ...
    f"Platform  : ThunderCompute A100 80GB",
...
if os.path.exists(ATROUS_BEST):
    shutil.copy(ATROUS_BEST, os.path.join(BASE_DIR, "atrousmamba_cbam_p2head_best.pt"))

_zip_path = os.path.join(BASE_DIR, "atrousmamba_results.zip")
_zip_cmd = (
    f"cd {BASE_DIR} && zip -r atrousmamba_results.zip "
    f"excel_reports/ plots/ benchmark_reports/ "
    f"atrousmamba_cbam_p2head_best.pt "
    f"gradient_norms.csv 2>/dev/null || true"
)
subprocess.run(_zip_cmd, shell=True)
print(f"✓ Results packaged → {_zip_path}")

print("\n" + "=" * 80)
print("ALL DONE — AtrousMamba+CBAM+P2 experiment complete!")
print("=" * 80)
print(f"\nDownload from ThunderCompute instance:")
print(f"  tnr scp 0:{_zip_path} ./")
print(f"  tnr scp 0:{ATROUS_BEST} ./")
print(f"  tnr scp 0:{CKPT_DIR}/session_last.pt ./")
print(f"\nDon't forget to DELETE the instance when done (stops billing)!")

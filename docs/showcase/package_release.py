...
def main():
    ...
    allowed_dirs = {"analysis", "assets", "demos", "docs", "results"}
    allowed_suffixes = {".md", ".svg", ".png", ".jpg", ".csv", ".json", ".txt", ".mp4"}
    ...
    for p in files:
        ...
        if p.suffix in {".md", ".csv", ".json", ".py", ".txt", ".svg"}:
            s = p.read_text(encoding="utf-8-sig")
            if re.search(r"Mofazzal|Hosen|Hossen|2007074|CSER[- ]?26[- ]?88|Masudul|Ahsan|Khulna|KUET", s, re.I):
                raise ValueError(f"Personal or academic identifier in export: {rel}")
    ...
    root_readme = root_readme.replace("From this folder, with Python", "From the repository root, with Python")
    ...
    manifest = {"version": "1.0.2", "prepared": "2026-09-20", "scope": "selected results, diagrams, and recorded demos",
    ...
    target = ROOT / "pendrive/RESULTS_SHOWCASE"
    ...
    zip_path = ROOT / "pendrive/RESULTS_SHOWCASE.zip"
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=6) as archive:
        for p in sorted(target.rglob("*")):
            if p.is_file():
                archive.write(p, "RESULTS_SHOWCASE/" + p.relative_to(target).as_posix())

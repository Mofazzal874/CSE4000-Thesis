...
imagery with a compact model intended for deployment. Starting from a YOLO11m
...
detectors reported in the original C2A study, only YOLOv9-e scores higher, and
it has roughly three times as many parameters. The whole
...
stalled, the next measurable gain came from training supervision: a Wasserstein
blend inside the label assigner raised sub-8-pixel recall in every training
seed tested (mean $+1.6$ points; $+2.08$ in the matched pair, bootstrap
interval excluding zero), while a frequency-gated
...
significant. In both the assignment and adaptation experiments, changing the
supervision had a larger measured effect than adding another feature branch.

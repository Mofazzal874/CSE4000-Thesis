...
\begin{longtable}{@{}c l l l@{}}
  ...
  \midrule
  \endfirsthead
  \multicolumn{4}{@{}l}{Table~\ref{tab:layers} (Continued.) Layer configuration of
  the proposed detector at 640-px input.}\\[2pt]
  \toprule
  Layers & Module (arguments) & Output & Role \\
  \midrule
  \endhead
...
\begin{figure}[tbp]
  \centering
  \figorplaceholder{figures/figures_png_format/stride_problem_v2.png}{7.5cm}
...
\end{equation}
and the normalised Wasserstein similarity of Wang et al.~\cite{wang2021nwd, xu2022nwdrka}
maps it into $(0,1]$ as
\begin{equation}
...
\begin{figure}[tbp]
  ...
  \caption[Slicing-aided hyper inference pipeline.]{Slicing-aided hyper
  inference. The input frame is divided into
  overlapping tiles and also passed whole; the detector runs on each tile at
  close to its training resolution, and the per-tile predictions are merged by
  greedy non-maximum merging using an intersection-over-smaller criterion, which
  suits boxes cut by a tile border.}
  \label{fig:sahipipe}
\end{figure}
...
\begin{figure}[tbp]
  ...
  \caption[Test-time augmentation pipeline.]{Test-time augmentation. The trained
  weights are unchanged; the frame
  is passed a fixed number of times at several input scales and the resulting
  detections are fused, so the cost is bounded rather than growing with tile
  count as it does for slicing.}
  \label{fig:ttapipe}
\end{figure}

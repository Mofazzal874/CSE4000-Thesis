...
\begin{figure}[tbp]
  ...
  \caption{High-level schematic of the proposed detection pipeline. An aerial
...
\begin{figure}[tbp]
  ...
  \caption{Layer-level architecture of the proposed YOLO11m~+~CBAM~+~P2 detector
...
\begin{table}[tbp]
  \centering
  \caption{Layer configuration of the proposed detector (640-px input). Channel
...
\begin{figure}[tbp]
  ...
  \begin{minipage}{0.44\linewidth}\centering
    ...
    {\small (d) detections}
  \end{minipage}
  \caption{The trained CBAM~+~P2 detector on the running-example collapsed-building
...
\begin{figure}[tbp]
  ...
  \caption{Effect of the CBAM substitution. The schematic tiles (left) illustrate
...
\begin{figure}[tbp]
  ...
  \caption{The stride problem motivating the P2 head: the same $\sim$12-px human
  under each scale's detection grid. All four panels show one $32\times32$-px
  image patch (a single P5 cell); the target covers about 0.4 of a P5 cell, 0.75
  of a P4 cell, and 1.5 P3 cells, but roughly three cells of the added stride-4
  P2 grid (red), enough for precise localisation.}
...
\begin{figure}[tbp]
  ...
  \caption{The SAHI inference pipeline on a C2A scene: the input image is sliced
...
\begin{figure}[tbp]
  ...
  \caption{The test-time augmentation pipeline: the input image is evaluated at

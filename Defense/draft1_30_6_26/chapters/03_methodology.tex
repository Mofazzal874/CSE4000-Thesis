...
The complete layer-level architecture of the proposed detector is shown in
Figure~\ref{fig:fullarch}. The backbone follows the YOLO11m layout (layers 0--9),
with the CBAM block at layer~10 in place of the original C2PSA attention. The neck
runs a top-down pass (layers 11--19) that now extends one level further than the
standard design, up to the stride-4 P2 resolution, and a bottom-up pass (layers
20--28) that re-aggregates all four levels. The head (layer~29) decodes boxes at
the four scales, and the elements added or modified in this work are marked in the
figure. The subsections that follow describe each modification.

\begin{figure}[p]
  \centering
  \figorplaceholder{figures/fig_cbam_p2_architecture.png}{19cm}
  \caption{Layer-level architecture of the proposed YOLO11m~+~CBAM~+~P2 detector.
  The backbone (layers 0--9) extracts features at five resolutions; CBAM at
  layer~10 replaces the original C2PSA block; the neck fuses four levels through a
  top-down pass (11--19), whose new stride-4 branch (17--19) produces the
  $160\times160\times128$ P2 feature, and a bottom-up pass (20--28); the head
  detects at strides 4, 8, 16, and 32, and NMS produces the final boxes.
  Red-dashed elements are the modifications introduced in this work.}
  \label{fig:fullarch}
\end{figure}

...
\begin{figure}[tbp]
  ...
  \caption[The CBAM module.]{The CBAM module. Channel attention (from global average and max pooling
...
\begin{figure}[tbp]
  ...
  \caption[Construction of the added P2 branch.]{Construction of the added P2 branch. The P3 neck feature is upsampled
  and concatenated with the high-resolution backbone P2 feature (a skip connection),
  then fused by a C3k2 block into a $160\times160$ feature that a stride-4 detection
  head reads. Each P2 grid cell covers about $4\times4$ input pixels, so the branch
  resolves targets the coarser P3--P5 scales miss.}
  \label{fig:p2}
\end{figure}
...
\begin{figure}[tbp]
  ...
  \caption[The explored C3k2Mamba block.]{The explored C3k2Mamba block: forward and reverse selective scans over a
...
\begin{figure}[tbp]
  ...
  \caption[The additive ablation studied in this thesis.]{The additive ablation studied in this thesis: each step changes one
...
\begin{table}[tbp]
  \centering
  \caption[Position of this thesis relative to prior work.]{Position of this thesis relative to prior work on small-object aerial
  human detection.}
  \label{tab:gap}

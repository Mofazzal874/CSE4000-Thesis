...
FIXED VERSION: Training already completed. This script loads pre-trained
P2-Head results from uploaded Kaggle input and generates all benchmark
reports, plots, and Excel files.
...
PRE-TRAINED MODELS (uploaded to Kaggle input):
  - Baseline:  /kaggle/input/<dataset>/runs/detect/yolo11m_baseline/weights/best.pt
  - P2-Head:   /kaggle/input/yolo11m-p2head/runs/detect/yolo11m_p2head/weights/best.pt
...
for root, dirs, files in os.walk("/kaggle/input"):
    ...
    if os.path.isfile(candidate):
        BASELINE_INPUT_DIR = root
        print(f"  ✓ Found baseline dataset at: {BASELINE_INPUT_DIR}")
        break

if BASELINE_INPUT_DIR is None:
    print("  ❌ Could not auto-detect baseline! Listing /kaggle/input/ contents:")
    for root, dirs, files in os.walk("/kaggle/input"):
...
BASELINE_BEST = f"{BASELINE_INPUT_DIR}/runs/detect/yolo11m_baseline/weights/best.pt"
BASELINE_RESULTS_CSV = f"{BASELINE_INPUT_DIR}/runs/detect/yolo11m_baseline/results.csv"

# Auto-detect the Kaggle input dataset containing P2-Head results
P2HEAD_INPUT_DIR = None
_P2_SEARCH_FILE = os.path.join("runs", "detect", "yolo11m_p2head", "weights", "best.pt")
print("\nSearching for pre-trained P2-Head in /kaggle/input/...")

for root, dirs, files in os.walk("/kaggle/input"):
    candidate = os.path.join(root, _P2_SEARCH_FILE)
    if os.path.isfile(candidate):
        P2HEAD_INPUT_DIR = root
        print(f"  ✓ Found P2-Head dataset at: {P2HEAD_INPUT_DIR}")
        break

if P2HEAD_INPUT_DIR is None:
    print("  ❌ Could not auto-detect P2-Head! Listing /kaggle/input/ contents:")
    for root, dirs, files in os.walk("/kaggle/input"):
        level = root.replace("/kaggle/input", "").count(os.sep)
        if level < 4:
            indent = "     " + "  " * level
            ...
            for f in files[:5]:
                print(f"{indent}  📄 {f}")
    raise FileNotFoundError("Pre-trained P2-Head not found! Upload yolo11m-p2head dataset.")

P2HEAD_BEST = f"{P2HEAD_INPUT_DIR}/runs/detect/yolo11m_p2head/weights/best.pt"
P2HEAD_RESULTS_CSV = f"{P2HEAD_INPUT_DIR}/runs/detect/yolo11m_p2head/results.csv"

# Validate all paths
for path, label in [
    (BASELINE_BEST, "Baseline best.pt"),
    (BASELINE_RESULTS_CSV, "Baseline results.csv"),
    (P2HEAD_BEST, "P2-Head best.pt"),
    (P2HEAD_RESULTS_CSV, "P2-Head results.csv"),
]:
...
# ============================================================================
# CELL 4: Load Pre-trained Models (SKIP TRAINING)
# ============================================================================
...
print("\n" + "=" * 80)
print("PHASE 2/2: LOADING PRE-TRAINED P2-HEAD (from uploaded input)")
print("=" * 80)
assert os.path.exists(P2HEAD_BEST), f"P2-Head model not found: {P2HEAD_BEST}"
print(f"✓ P2-Head loaded from: {P2HEAD_BEST}")
print("⏩ Training SKIPPED — using pre-trained weights from uploaded results")


# ============================================================================
# CELL 5: Output Directories Setup
# ============================================================================
import os
import pandas as pd
import matplotlib.pyplot as plt
...
# Both results from uploaded input
BASELINE_RUN = f"{BASELINE_INPUT_DIR}/runs/detect/yolo11m_baseline"
P2HEAD_RUN = f"{P2HEAD_INPUT_DIR}/runs/detect/yolo11m_p2head"

print(f"✓ Output directories ready")
print(f"  Baseline run: {BASELINE_RUN}")
print(f"  P2-Head run:  {P2HEAD_RUN}")


# ============================================================================
# CELL 6: Training Curves with TOTAL LOSS
# ============================================================================
...
# ============================================================================
# CELL 7: Model Complexity Comparison
# ============================================================================
from ultralytics import YOLO

baseline_model = YOLO(BASELINE_BEST)
p2head_model = YOLO(P2HEAD_BEST)
...
# ============================================================================
# CELL 8: Ultralytics Val() for Official mAP Metrics
# ============================================================================
print("\n" + "=" * 80)
...
# ============================================================================
# CELL 9: Helper Functions for Custom Evaluation
# ============================================================================
import cv2
...
# ============================================================================
# CELL 10: Comprehensive Evaluation Function
# ============================================================================
...
# ============================================================================
# CELL 11: Visualization & Advanced Metrics Functions
# ============================================================================
...
# ============================================================================
# CELL 13: Validation Set Evaluation
# ============================================================================
...
# ============================================================================
# CELL 15: Per-Size Recall Bar Chart (Key Ablation Visual)
# ============================================================================
...
# ============================================================================
# CELL 16: Final Summary & Master Report
# ============================================================================
print("\n" + "=" * 80 + "\nFINAL SUMMARY\n" + "=" * 80)

"""
yolov9s_final_month.py
======================

Supplementary benchmark of YOLOv9s on C2A under the final-month metric protocol
defined in `Last Month/system_spec.md`. NOT part of the Phase A/B/C ablation
matrix (Section 9.7 of the spec lists YOLOv9 as a cited-not-retrained baseline);
this run is kept for continuity with the earlier Kaggle benchmarks and as a
sanity reference for the YOLO11m headline.

Spec compliance summary (sections deliberately implemented):
  - Sections 3, 7   : single-GPU 16 GB defaults, AMP, BF16 preference
  ...
  - Section 11      : full metric catalog (detection / training-dyn / efficiency
                      ...
                      metrics (Section 11.6) that don't apply to vanilla v9s
...
Explicitly SKIPPED (with reasons logged to skipped_metrics.txt):
  - attention_map_examples / channel_attention_weights  (no CBAM in v9s)
  - per-stride_AP / tiny_obj_recall_by_head             (no P2 head in v9s)
...
MODEL_TAG          = "yolov9s_supplementary"     # snake_case, used in run_id (Section 16)
PRETRAINED_WEIGHTS = "yolov9s.pt"                 # Ultralytics auto-downloads
SEEDS              = [0]                          # final-month protocol wants 5;
                                                  # default to 1 (see comment at __main__)

# Training
NUM_EPOCHS         = 300                          # upper bound only (Section 18.1)
PATIENCE           = 30                           # Ultralytics fitness patience
F2_PATIENCE        = 20                           # custom F2-based patience
IMG_SIZE           = 640
BATCH_SIZE         = 32                           # 4070 Ti SUPER, v9s + AMP (Section 3)
...
# Auto-detect class list from the COCO JSON; fall back to ['person'] per spec.
def detect_class_names() -> List[str]:
    ...
    if j.is_file():
        try:
            ...
            names = [c.get("name", f"cls_{c.get('id','?')}") for c in cats]
            if names:
                return names
...
def system_sanity() -> Dict[str, Any]:
...
SANITY = system_sanity()
torch.backends.cudnn.benchmark = True

# Free-space check (Section 5.5)
def check_free_space(min_gb: int = 20):
    if HAS_PSUTIL:
        try:
...
def freeze_or_verify_splits() -> Dict[str, str]:
    splits = {
...
SPLIT_MD5 = freeze_or_verify_splits()
...
def write_env_json(path: Path, *, run_id: str, seed: int,
    ...
    env = {
        "run_id":             run_id,
        "model_tag":          MODEL_TAG,
        "in_ablation_matrix": False,
        "matrix_status":      ("supplementary -- YOLOv9s is Section 9.7 "
                               "cited-not-retrained; this run is for "
                               "continuity sanity only"),
...
def run_one_seed(seed: int) -> Dict[str, Any]:
    ...
    # Resume detection (Section 20.2)
    last_pt = dirs["weights"] / "last_safety.pt"
    resume = last_pt.is_file()
    if resume:
        print(f"[resume] found {last_pt} -- enabling resume")
    ...
    skipped.extend([
        "[SKIPPED] attention_map_examples         -- vanilla YOLOv9s (no CBAM)",
        "[SKIPPED] channel_attention_weights      -- vanilla YOLOv9s (no CBAM)",
        "[SKIPPED] spatial_attention_entropy      -- vanilla YOLOv9s (no CBAM)",
        "[SKIPPED] per-stride_AP                  -- vanilla YOLOv9s (no P2 head)",
        "[SKIPPED] tiny_obj_recall_by_head        -- vanilla YOLOv9s (no P2 head)",
        "[SKIPPED] ssm_state_norm                 -- vanilla YOLOv9s (no SSM)",
        "[SKIPPED] forward_vs_backward_scan_disag -- vanilla YOLOv9s (no SSM)",
        "[SKIPPED] window_size_per_layer          -- vanilla YOLOv9s (no SSM)",
        "[SKIPPED] dilation_branch_contribution   -- vanilla YOLOv9s (no SSM)",
        "[SKIPPED] injection_layer_indices        -- vanilla YOLOv9s (no SSM)",
...
**Status:** Supplementary benchmark. NOT a row in the final-month
Phase A/B/C ablation matrix (system_spec.md Section 9.6).
...
## Limitations
- Single seed only (SEEDS={SEEDS}) -- variance / significance estimates require
  multi-seed (Section 12.1 of system_spec.md).
- Architecture-specific metrics in Section 11.6 are N/A for vanilla YOLOv9s.
...
""")
    ...
    manifest = {
        "run_id":            run_id,
        "model_tag":         MODEL_TAG,
        "ablation_matrix_row": None,
        "matrix_status":     "supplementary",
        "parent_run_id":     None,
        "git_commit":        GIT_HASH,
        "git_dirty":         GIT_DIRTY,
...
def main():
    print("\n" + "=" * 78)
    print(f"yolov9s_final_month.py  --  MODEL_TAG={MODEL_TAG}  SEEDS={SEEDS}")
    ...
    # Multi-seed roll-up (only meaningful with >=2 seeds; Section 12.1 wants 5)
    if len(all_summaries) >= 1:
        roll = OUTPUT_ROOT / "runs" / f"{MODEL_TAG}_multi_seed_rollup.json"
        roll.write_text(json.dumps(all_summaries, indent=2, default=str))
        print(f"[main] roll-up written to {roll}")
    if len(SEEDS) < 5:
        print(f"\n[main] NOTE: only {len(SEEDS)} seed(s) run. system_spec.md "
              f"Section 12.1 requires >=5 seeds for any 'improvement' claim. "
              f"Set SEEDS=[0,1,2,3,4] in the config block when you have the "
              f"wall-clock budget (~30-50 h on this GPU).")
